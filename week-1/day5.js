...
function pickPun(string) {
  //  let bannedWords = ["dog", "bark", "bone"];
  let stringArray = string.split(",");
  console.log(
    stringArray
      .filter((e) => e.trim() === e)
      .filter((e) => e.toLowerCase().includes("dog") === false)
      .filter((e) => e.toLowerCase().includes("bark") === false)
      .filter((e) => e.toLowerCase().includes("bone") === false)
      .filter((e) => e.length % 5 !== 0)
      .filter((e) => (e.charCodeAt(0) + e.charCodeAt(e.length - 1)) % 2 !== 0)
      .filter((e) => e.charAt(Math.ceil(e.length / 2)) !== "e")
      .filter((e) => {
        let sum = 0;
...
//let test = "The Pawshank Redemption";
//console.log((test.charCodeAt(0) + test.charCodeAt(test.length - 1)) % 2);
